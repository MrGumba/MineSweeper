...
def menu(hardness = 1, win = 0, ai = 0, winCount = 0, roundCount = 0, lossCount = 0):
    ...
    while True: # Loopar tills ett correkt värde har angäts.
        try:
            if roundCount < 1 or roundCount >= 149:
                print('Du har vunnit', winCount, 'gånger och förlorat', lossCount, 'gånger')
                menuSelect = int(input('Skriv in vilket meny alternativ du vill välja (1, 2, 3 eller 4): '))
            elif roundCount < 149:
                menuSelect = 4
            if menuSelect == 1:
                fullListMaker(hardness, ai) # Anropar fullListMaker som gör fulla listan som sedan startar igång resten av pogrammet.
            elif menuSelect == 2:
                difficulty() # Anropar Difficulty där man ändrar svårigheten av spelet.
            elif menuSelect == 3:
                exit()  # Ifall man ångrar sig och vill lämna.
            elif menuSelect == 4:
                while roundCount < 149:
                    ai = 1
                    fullListMaker(hardness, ai, winCount, roundCount, lossCount)  # Ifall man ångrar sig och vill lämna.
            else:
...
def fullListMaker(hardness, ai, winCount, roundCount, lossCount):
    if hardness == 1:
        ...
        playerMap = [[], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []]
    fieldMaker(hardness, fullList, playerMap, ai, winCount, roundCount, lossCount) # Anropar fieldMaker som skapar fältet med siffror. 



'''Denna funktion skapar spelfältet samt lägger ut bomberna för spelfältet '''
def fieldMaker(hardness, fullList, playerMap, ai, winCount, roundCount, lossCount):
    ...
    bombCalculator(hardness, fullList, playerMap, ai, winCount, roundCount, lossCount) # Anropar bombCalculator för att räkna ut siffrorna som ska vara runt alla bomber.



''' Räknar ut vilka numer som ska vara runt en bomb genom att +1 alla siffror runt en bomb'''
def bombCalculator(hardness, fullList, playerMap, ai, winCount, roundCount, lossCount):
            ...
            if fullList[i][k] == 'x': # Ifall den hittar en bomb (markerat som 'x') så gör den följande.
                for j in range(back, front):  # Kollar vänster höger och mitt så länge inget spesefikt värde angätts.
                    ...
                    if k + 1 < len(fullList) and fullList[i - 1 + j][k + 1] != 'x': # Kollar/Ändrar höger.
                        fullList[i - 1 + j][k + 1] += 1
    mapMaker(hardness, fullList, playerMap, ai, winCount, roundCount, lossCount) # Anropar mapMaker för att skapa kartan spelaren ser.
...
def mapMaker(hardness, fullList, playerMap, ai, winCount, roundCount, lossCount):
    ...
    if hardness == 3: # Bomberna och toma utrymet för denna svårighetsgrad.
        freeSpace = [477]
        bombCount = [99]        
    mineHub(hardness, playerMap, fullList, freeSpace, bombCount, ai, winCount, roundCount, lossCount) # Anropar mineHub som är hubben för hela spelet.




'''mineHub är hubben som Anropar funktionerna som aktivts behövs medans man köra spelet'''
def mineHub(hardness, playerMap, fullList, freeSpace, bombCount, ai, winCount, roundCount, lossCount):
    while freeSpace[0] > 0: # Kollar så spelet är inte redan över.
        mapPrint(hardness, playerMap, bombCount, fullList, ai, winCount, roundCount, lossCount) # Anropar mapPrint för att visa kartan för spelaren.
        playerInput = playerChoice(hardness, fullList, playerMap, ai, winCount, roundCount, lossCount, freeSpace) # Anropar playerChoice för att få veta vad användaren vill göra. 
        mapReveal(playerInput, playerMap, fullList, freeSpace, bombCount, ai, winCount, roundCount, lossCount) # Anropar mapReveal som ändrar till kartan efter användarens input.
    if freeSpace[0] <= 0: # Kollar ifall det är inga utrymen kvar där man vinner ifall inga är kvar (som inte är bomber).
        roundCount += 1
        winCount += 1
        menu(1, 'W', winCount = winCount, roundCount = roundCount, lossCount = lossCount) # Anropar menu för att gå till menyn med informationen att användaren har vunnit.


'''MapPrint printar ut kartan som användaren ska se samt mängden bomber som finns. '''
def mapPrint(hardness, playerMap, bombCount, fullList, ai, winCount, roundCount, lossCount):
    easyNumber = []
...
def playerChoice(hardness, fullList, playerMap, ai, winCount, roundCount, lossCount, freeSpace):
    ...
    while True:
        try:
            if ai == 0:
                ...
                playerInput = aiInput(hardness, fullList, playerMap, ai, winCount, roundCount, lossCount, freeSpace)
            ...
            if playerInput[0] == 'f' and playerMap[int(playerInput[2])][int(playerInput[1])] == '#': # Kollar ifall användaren flaggade en position.
                ...
                return playerInput
        except (IndexError): # Skickar denna felkod ifall användaren inte ger ut två heltal som värde.
...
def mapReveal(playerInput, playerMap, fullList, freeSpace, bombCount, ai, winCount, roundCount, lossCount):
    if playerInput[0] == 'f': # Denna är tillför att flagga en position.
        ...
        roundCount += 1
        lossCount += 1        
        menu(1, 'L', winCount = winCount, roundCount = roundCount, lossCount = lossCount) # Anropar menu med informationen att användaren förlorade.

    elif fullList[int(playerInput[1])][int(playerInput[0])] == 0: # Kollar ifall platsen den ska visa var en nolla.
        zeroChecker(playerInput, freeSpace, playerMap, fullList, ai) # Anropar zeroChecker för att vissa alla tal runt nollorna som visades.
    else: 
...
def zeroChecker(playerInput, freeSpace, playerMap, fullList, zeroFinderX = 0, zeroFinderY = 0):
    ...
    for j in range(back, front): # Kollar listan ett up och ett steg ner i listorna.
        ...
        if int(playerInput[0]) - 1 + zeroFinderX > -1 and playerMap[int(playerInput[1]) - 1 + j + zeroFinderY][int(playerInput[0]) - 1 + zeroFinderX] == '#' and fullList[int(playerInput[1]) - 1 + j + zeroFinderY][int(playerInput[0]) - 1 + zeroFinderX] != 'x': 
        ...
        if int(playerInput[0]) + zeroFinderX < 10 and playerMap[int(playerInput[1]) - 1 + j + zeroFinderY][int(playerInput[0]) + zeroFinderX] == '#' and fullList[int(playerInput[1]) - 1 + j + zeroFinderY][int(playerInput[0]) + zeroFinderX] != 'x':
        ...
        if int(playerInput[0]) + zeroFinderX + 1 < len(fullList) and playerMap[int(playerInput[1]) - 1 + j + zeroFinderY][int(playerInput[0]) + 1 + zeroFinderX] == '#' and fullList[int(playerInput[1]) - 1 + j + zeroFinderY][int(playerInput[0]) + 1 + zeroFinderX] != 'x':
            holder = str(fullList[int(playerInput[1]) - 1 + j + zeroFinderY][int(playerInput[0]) + 1 + zeroFinderX])
            playerMap[int(playerInput[1]) - 1 + j + zeroFinderY][int(playerInput[0]) + 1 + zeroFinderX] = holder
            freeSpace[0] -= 1 # Räknar att det finns 1 mindre plats kvar på planen.
            if playerMap[int(playerInput[1]) - 1 + j + zeroFinderY][int(playerInput[0]) + 1 + zeroFinderX] == '0':
...
def aiInput(hardness, fullList, playerMap, ai, winCount, roundCount, lossCount, freeSpace):
    foundSomething = 0
    for y in range(0, len(fullList) - 1): 
        for x in range(0, len(fullList) - 1):
            if playerMap[y][x] != '#' and playerMap[y][x] != 0 and playerMap[y][x] != '¤':
                squareCount = 0
                flagCount = 0
                back = 0 # back och front är till för att programmet ska gå ett steg upp från positionen användaren angav till ett steg under.
                front = 3            
                if y == 0: # ifall y värdet är 0 så ändras värdet så den inte kollar utanför listan.
                    back = 1
                elif y == len(playerMap) - 1: # ifall y värdet är lika stort som listan så ändras värdet så den inte kollar utanför listan.
                    front = 2 
                for j in range(back, front): # Kollar listan ett up och ett steg ner i listorna.
                    '''Kollar positionen ett steg till vänster i listan.'''
                    if x - 1 > -1 and playerMap[y - 1 + j][x - 1] == '#': 
                        squareCount += 1
                    if x - 1 > -1 and playerMap[y - 1 + j][x - 1] == '¤': 
                        flagCount += 1

                    '''Kollar positionen i mitten av det angavt x värde i listan.'''     
                    if playerMap[y - 1 + j][x] == '#':
                        squareCount += 1
                    if playerMap[y - 1 + j][x] == '¤':
                        flagCount += 1
                    

                    '''Kollar positionen ett steg till höger i listan.'''        
                    if x + 1 < len(fullList) and playerMap[y - 1 + j][x + 1] == '#':
                        squareCount += 1
                    if x + 1 < len(fullList) and playerMap[y - 1 + j][x + 1] == '¤':
                        flagCount += 1
                if int(playerMap[y][x]) - flagCount == 0:
                    foundSomething = 1
                    for j in range(back, front): # Kollar listan ett up och ett steg ner i listorna.
                        '''Kollar positionen ett steg till vänster i listan.'''
                        if x - 1 > -1 and playerMap[y - 1 + j][x - 1] == '#': 
                ...
                if int(playerMap[y][x]) - flagCount == squareCount:
                    for j in range(back, front): # Kollar listan ett up och ett steg ner i listorna.
                        '''Kollar positionen ett steg till vänster i listan.'''
                        if x - 1 > -1 and playerMap[y - 1 + j][x - 1] == '#':
                            foundSomething = 1
                            valueX = x - 1
                            valueY = y - 1 + j
                            playerInput = ['f', x - 1, y - 1 + j]
                            return playerInput
    
                        '''Kollar positionen i mitten av det angavt x värde i listan.'''     
                        if playerMap[y - 1 + j][x] == '#':
                            foundSomething = 1
                            playerInput = ['f', x, y - 1 + j]
                            return playerInput
        ...
        randomY = random.randint(0, len(fullList) - 1) # Yttre listan (Y led)
        randomX = random.randint(0, len(fullList) - 1) # Innre listan (X led)
        playerInput = [randomX, randomY]
        return playerInput
